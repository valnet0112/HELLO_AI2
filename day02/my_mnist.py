...
class MyRecom:
    ...
    def insertRecom(self, BR_DPT_ID, BR_EMP_INTEREST, BR_BD_ID, BR_RANK):  
        ...
        self.bookdatalist()
        for dpt in self.deptList:
            for interest in self.interestList:
                rank_pred=[]
                for rank in ranks:
                    emp_interest = np_utils.to_categorical(interest[1], self.interestLength, int)
                    dpt_label = np_utils.to_categorical(dpt[1], self.deptLength, int)
                    xt = np.append(emp_interest, dpt_label)
        sql = f"""
